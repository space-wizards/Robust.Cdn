...
internal sealed class ZStdCompressStream : Stream
{
    ...
    public async ValueTask FlushEndAsync()
    {
        _hasSession = false;
        await FlushInternalAsync(ZSTD_EndDirective.ZSTD_e_end);
    }
    ...
    private async ValueTask FlushInternalAsync(ZSTD_EndDirective directive, CancellationToken cancel = default)
    {
        var outBufPos = (nuint)_bufferPos;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var inBufPos = (nuint)0;
        var inBufSize = (nuint)buffer.Length;
        var outBufPos = (nuint)_bufferPos;
